            ...
            {navbarLink.map((item, index) => {
              ...
              return (
                ...
                >
                  <Link
                    to={item.url}
                    className={`group-hover:text-[#FD915A] flex flex-col items-center gap-y-1 ${active ? 'text-[#FD915A]' : ''}`}
                  >
                    <img
                      src={`assets/img/${item.icon}.png`}
                      alt={item.title}
                      className={` w-[22px] h-[22px] ${active ? 'hidden' : 'group-hover:hidden'}`}
                    />
                    {/* hover img */}
                    <img
                      src={`assets/img/${item.icon}-hover.svg`}
                      alt={item.title}
                      className={` w-[22px] h-[22px] ${active ? 'block' : 'hidden  group-hover:block'}`}
                    />
                    {item.title}
                  </Link>
                  <span className={`absolute bottom-0 w-full h-[5px] bg-[#FD915A] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-in-out ${active ? 'scale-x-100' : 'scale-x-0'}`}></span>
                </li>
              )
            })}
          </ul>
